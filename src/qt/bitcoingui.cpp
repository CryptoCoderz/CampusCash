 ...
 * The CampusCash Developers 2018-2020
 */
...
CampusCashGUI::CampusCashGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(900, 520);
    setWindowTitle(tr("CampusCash") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void CampusCashGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a CampusCash address"));
    ...
    aboutAction = new QAction(QIcon(fUseDarkTheme ? ":/icons/dark/bitcoin-dark" : ":/icons/bitcoin"), tr("&About CampusCash"), this);
    aboutAction->setToolTip(tr("Show information about CampusCash"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for CampusCash"));
    ...
    editConfigAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit CampusCash.conf"), this);
    editConfigAction->setToolTip(tr("Edit the configuration file for CampusCash"));
    editConfigExtAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit CampusCash.conf (external)"), this);
    editConfigExtAction->setToolTip(tr("Edit the configuration file for CampusCash (external editor)"));
    openDataDirAction = new QAction(QIcon(":/icons/folder"), tr("&Open data dir"), this);
    openDataDirAction->setToolTip(tr("Open the directory where CampusCash data is stored"));
...
void CampusCashGUI::createToolBars()
{
...
void CampusCashGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("CampusCash client") + QString(" ") + tr("[testnet]"));
...
void CampusCashGUI::setWalletModel(WalletModel *walletModel)
{
...
void CampusCashGUI::setMessageModel(MessageModel *messageModel)
{
...
void CampusCashGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("CampusCash client"));
...
#ifndef Q_OS_MAC
void CampusCashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CampusCashGUI::optionsClicked()
{
...
void CampusCashGUI::aboutClicked()
{
    AboutDialog dlg;
...
void CampusCashGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to CampusCash network", "", count));
}

void CampusCashGUI::setNumBlocks(int count)
{
    QString tooltip;
...
void CampusCashGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("CampusCash") + " - ";
...
void CampusCashGUI::error(const QString &title, const QString &message, bool modal)
{
...
void CampusCashGUI::changeEvent(QEvent *e)
{
...
void CampusCashGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void CampusCashGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(CampusCashUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void CampusCashGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    notificator->notify(Notificator::Information,
                        (amount)<0 ? tr("Sent transaction") :
                          ...
                          tr("Date: %1\n"
                             ...
                             "Address: %4\n")
                          .arg(date)
                          .arg(CampusCashUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                          .arg(type)
                          .arg(address), icon);
}

void CampusCashGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void CampusCashGUI::clearWidgets()
{
...
void CampusCashGUI::gotoMasternodeManagerPage()
{
...
void CampusCashGUI::gotoBlockBrowser()
{
...
void CampusCashGUI::gotoOverviewPage()
{
...
void CampusCashGUI::gotoHistoryPage()
{
...
void CampusCashGUI::gotoAddressBookPage()
{
...
void CampusCashGUI::gotoReceiveCoinsPage()
{
...
void CampusCashGUI::gotoSendCoinsPage()
{
...
void CampusCashGUI::gotoSignMessageTab(QString addr)
{
...
void CampusCashGUI::gotoVerifyMessageTab(QString addr)
{
...
void CampusCashGUI::gotoMessagePage()
{
...
void CampusCashGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CampusCashGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid CampusCash address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void CampusCashGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void CampusCashGUI::setEncryptionStatus(int status)
{
...
void CampusCashGUI::encryptWallet()
{
    if(!walletModel)
        return;
...
void CampusCashGUI::backupWallet()
{
...
void CampusCashGUI::importPrivateKey()
{
...
void CampusCashGUI::changePassphrase()
{
...
void CampusCashGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void CampusCashGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void CampusCashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CampusCashGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void CampusCashGUI::updateStakingIcon()
{
    updateWeight();
...
void CampusCashGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void CampusCashGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void CampusCashGUI::editConfigExt()
{
...
void CampusCashGUI::openDataDir()
{
