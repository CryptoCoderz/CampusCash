...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CCampusCashAddress(redeemScript.GetID()).ToString();
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_NOT10000IFMN) {
                    found = !(fMasterNode || pcoin->vout[i].nValue == MasternodeCollateral(pindexBest->nHeight)*COIN || pcoin->vout[i].nValue == MasternodeCollateral_Tier_2(pindexBest->nHeight)*COIN);
                } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN){
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    if(pcoin->vout[i].nValue != MasternodeCollateral(pindexBest->nHeight)*COIN && pcoin->vout[i].nValue != MasternodeCollateral_Tier_2(pindexBest->nHeight)*COIN) found = pcoin->vout[i].nValue; // do not use any MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++){
                ...
                if (pcoin->vout[i].nValue == MasternodeCollateral_Tier_2(pindexBest->nHeight)*COIN){

                    //LogPrintf("CWallet::AvailableCoinsForStaking - Found Masternode collateral tier 2.\n");
                    found = true;
                    break;
                }
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-CampusCash-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CCampusCashAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CCampusCashAddress addrTo(ckidTo);
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CCampusCashAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CCampusCashAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CCampusCashAddress(address).ToString());
}
...
bool CWallet::ImportPrivateKey(CCampusCashSecret vchSecret, string strLabel, bool fRescan) {
    if (fWalletUnlockStakingOnly)
        return false;
