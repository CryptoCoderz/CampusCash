...
int nAnonymizeCampusCashAmount = 1000;
...
#else
    const char* pszModule = "CampusCash";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CampusCash.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "CampusCash.conf";
               ...
               fprintf(ConfFile, "port=19430\n");
               fprintf(ConfFile, "rpcport=19430\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=192.168.1.128:19430\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "CampusCashd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong CampusCash will not work properly.");
